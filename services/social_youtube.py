...
# --- PILLOW COMPATIBILITY PATCH (CRITICAL) ---
# This fixes the 'module PIL.Image has no attribute ANTIALIAS' error
...
def generate_karaoke_clips(words, font_path_preferred, fontsize=75, box_size=(900, 600), position=('center', 0.55)):
    ...
    for chunk in chunks:
        ...
        line_height = (font.getbbox("Ay")[3] - font.getbbox("Ay")[1]) * 1.4
...
def create_premium_reel(image_paths: list, hook_count: int, text_overlay: str, full_narrative: str, output_path: str = "output_premium.mp4", autonomous: bool = False):
    try:
            ...
            for i, img_path in enumerate(image_paths):
                try:
                    with Image.open(img_path) as pil_img:
                        # Using Image.LANCZOS directly for safety
                        pil_img = pil_img.resize((1188, 2112), Image.LANCZOS)
                        left, top = (1188 - 1080)/2, (2112 - 1920)/2
                        pil_img = pil_img.crop((left, top, left+1080, top+1920))
                        temp_path = f"temp_opt_{i}.jpg"
                        pil_img.save(temp_path, quality=90)
                    
                    clip = ImageClip(temp_path).set_duration(avg_duration + trans_duration)
                    clip = clip.resize(lambda t: 1.0 + (0.05 * t / (avg_duration + trans_duration))) 
                    clip = clip.set_position('center')
                    if i > 0: clip = clip.crossfadein(trans_duration)
                    clips.append(clip)
        ...
        if os.path.exists("voice.json"):
            ...
            if os.name == 'posix': font_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf"
            caption_clips = generate_karaoke_clips(words, font_path, fontsize=80, box_size=(950, 600), position=('center', 0.60))

        # 4. Composite
        hook_np = create_text_image_pil(text_overlay.upper(), fontsize=100, color=(255, 255, 255))
        hook_title = ImageClip(hook_np).set_duration(8).set_pos(('center', 200)).resize(lambda t: 1.0 + 0.005*t)

        render_size = (1080, 1920)
        layers = [video_track.resize(render_size), hook_title] + caption_clips
        ...
        final_video_master.write_videofile(
            output_path, fps=30, codec="libx264", preset="veryfast", threads=2,
            bitrate="8000k", audio=True, audio_codec="aac",
            ffmpeg_params=["-crf", "23", "-tune", "fastdecode"]
        )
